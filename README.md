...
**Ver.2から、ヤフオク!の類似商品の落札実績を自動で検索し、価格算出の根拠として使うようになりました。**
落札実績が見つかった場合はそれを主な根拠に、見つからない場合はVer.1同様にAIの推定のみで価格を算出します。
...
- Sharp(LINEから受信した画像のリサイズ・再圧縮)
- Cheerio(ヤフオク!落札相場ページのHTML解析)
- Railwayへのデプロイを想定
...
1. [Railway](https://railway.app/) で新規プロジェクトを作成し、このリポジトリ(GitHub連携)を
   接続します。Nixpacksが `package.json` を検出し、`npm install` → `npm run build` →
   `npm start` で自動的にビルド・起動します。
...
「分析開始」時に、商品情報(ブランド・商品名・型番)からヤフオク!の検索キーワードを組み立て、
`src/marketplace/yahooAuctionSearchProvider.ts` がヤフオク!の
[「落札相場・落札価格」検索ページ](https://auctions.yahoo.co.jp/closedsearch/) (`closedsearch`)
を取得します。取得した落札実績はOpenAIへのプロンプトに含め、価格算出の根拠として使わせています。

このページはサーバーサイドでレンダリングされた通常のHTMLなので、ブラウザを起動する必要はなく、
`fetch` + `cheerio` で軽量に取得しています(Ver.2の初期実装ではメルカリの検索結果ページを
Playwrightで取得する方式を試しましたが、データセンターからの自動アクセスがメルカリ側に検知され
安定して取得できなかったため、ヤフオク!に切り替えました)。

**重要な注意点:**

- `auctions.yahoo.co.jp/robots.txt` では検索系パスの多くが `Disallow` されていますが、
  この「落札相場」ページ(`/closedsearch/closedsearch`)は明示的に `Allow` されています。
  とはいえ自動アクセスであることに変わりはなく、個人利用・低頻度の範囲に留めてください。
- ヤフオク!とメルカリでは相場が異なる場合があります。プロンプト上もその旨をAIに伝えていますが、
  あくまで目安としてご利用ください。
- ページ構造が変わると動かなくなる可能性があります。定期的な動作確認が必要です。
...
- 実装の抽象化のため `MarketplaceSearchProvider` インターフェース([`src/marketplace/types.ts`](src/marketplace/types.ts))
  を介しています。別のデータソースに切り替える場合は、この実装を差し替えるだけで済みます。
...
- **相場が毎回「実績なし」になる**: ヤフオク!側のページ構造が変わった可能性があります。
  `src/marketplace/yahooAuctionSearchProvider.ts` の `data-cl-params` 解析ロジックが
  現在のページと一致しているか確認してください。検索に失敗してもAI推定のみで結果は返ります。
